...
const explainerGridItems = [
    { title: "Comprehensive 1-on-1 Consultation", imageHint: "tablet checklist coffee", imageUrl: "/1A_consultation.png", text: "Sit down (virtually or in-person) with a certified appraiser who listens to your goals, reviews key documents, and tailors the assignment to your specific property. You leave the meeting knowing exactly what to expect and how we'll deliver it." },
    { title: "Expert Valuation Team", imageHint: "team professionals", imageUrl: "/2B_valuation_team.png", text: "Your report is prepared and double-checked by a team with decades of combined experience, state certifications, and deep knowledge of Florida's sub-markets—so underwriters can trust every line." },
    { title: "Mobile Inspection Technology", imageHint: "tablet inspection", imageUrl: "/3C_mobile_inspection.png", text: "Our field inspectors capture HD photos, 3-D scans, and measurements in real time, syncing straight to the cloud. This cuts days off turnaround and gives you crystal-clear interior and exterior detail." },
    { title: "Laser Accurate Measurements", imageHint: "laser measure device", imageUrl: "/4D_laser_measurements.png", text: "We use commercial-grade laser devices and digital floor-plan tools to measure within ±2 mm. The result: square footage you and your lender can rely on—no tape-measure guesswork." },
    { title: "Statistically Modeled Analysis", imageHint: "data charts graphs", imageUrl: "/5_statistical_analysis.png", text: "Beyond simple comps, we benchmark your property against thousands of recent sales using regression modeling and market-trend data, delivering an evidence-backed value you can defend." },
    { title: "AI-Powered Reports", imageHint: "ai interface report", imageUrl: "/6_ai_reports.png", text: "Our AI engine flags outliers, checks UAD compliance, and formats the final report so it sails through underwriting—getting you answers faster and with fewer revision requests." },
];
...
export default function ServiceClientPage({ offering }: ServiceClientPageProps) {
  ...
  useEffect(() => {
    console.log('useEffect running, offering.id:', offering.id);
    
    if (offering.id === 'appraisal') {
      console.log('Loading Lindy embed for appraisal page');
      
      // Try the correct Lindy embed format from your configuration
      const loadLindyEmbedCorrect = () => {
        console.log('Loading correct Lindy embed from api.lindy.ai...');
        ...
        try {
          // Use the exact script format you provided from Lindy
          const script = document.createElement('script');
          script.async = true;
          script.src = 'https://api.lindy.ai/api/lindyEmbed/lindyEmbed.js?a=jordan-pierce-roi-home-services';
          script.crossOrigin = 'use-credentials';
          
          script.onload = () => {
            console.log('Correct Lindy script loaded successfully from api.lindy.ai');
            // Check for widget after a delay
            setTimeout(() => {
              const lindyWidget = document.querySelector('[data-lindy]') || 
                                document.querySelector('.lindy-widget') ||
                                document.querySelector('#lindy-widget') ||
                                document.body.querySelector('iframe[src*="lindy"]');
              if (lindyWidget) {
                console.log('Lindy widget found and working!', lindyWidget);
              } else {
                console.log('Lindy script loaded but widget not found yet, checking window objects...');
                console.log('Available Lindy objects:', {
                  Lindy: (window as any).Lindy,
                  lindyEmbed: (window as any).lindyEmbed,
                  lindyWidget: (window as any).lindyWidget
                });
              }
            }, 3000);
          };
          ...
          script.onerror = (error) => {
            console.error('Correct Lindy script failed to load:', error);
            console.log('Falling back to alternative methods...');
            loadLindyFallback();
          };
          
          document.head.appendChild(script);
          console.log('Correct Lindy script added to head');
          ...
          setTimeout(() => {
            const lindyWidget = document.querySelector('[data-lindy]') || 
                              ...
                              document.querySelector('#lindy-widget') ||
                              document.body.querySelector('iframe[src*="lindy"]');
            if (!lindyWidget && !(window as any).Lindy && !(window as any).lindyEmbed) {
              console.log('Lindy failed to load after all attempts, enabling custom fallback');
              setLindyFailed(true);
            } else {
              console.log('Lindy appears to be working, disabling custom fallback');
              setLindyFailed(false);
            }
          }, 10000);
          
        } catch (error) {
          console.error('Correct Lindy embed failed:', error);
          loadLindyFallback();
        }
      };
      ...
      // Start with correct method, fallback if needed
      const timer = setTimeout(() => {
        console.log('Starting Lindy embed initialization...');
        loadLindyEmbedCorrect();
      }, 1000);
